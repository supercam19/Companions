...
execute as @a at @s if score @s cmp.identifier = @e[type=wolf,tag=cmp.companion,sort=nearest,limit=1] cmp.identifier run function companions:player_new_companion
execute as @a at @s unless score @s cmp.identifier = @e[type=wolf,tag=cmp.companion,sort=nearest,limit=1] cmp.identifier run function companions:find_owner